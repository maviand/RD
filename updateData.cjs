...
const contentPlan = {
  ...
  finance: [
    ...
    { title: "Cárceles Factoría Autosustentables en Campo Abierto", desc: "Trabajo penitenciario resarcitorio productivo y forzoso sin encierro urbano pasivo.", leader: "Procuraduría (PGR)", model: "Modelo Productivo Restitutivo Penitenciario", sDesc: "Reemplazo gradual completo de celdas hiper-hacinadas por campamentos aserraderos y centros o naves textiles donde las empresas privadas invierten bajo estricta vigilancia estatal abaratando el costo de un recluso logrando al fin sustento y repago automático penitenciario." },
    { title: "Tribunales IA de Agilidad Comercial", desc: "Disputas mercantiles resueltas sistemáticamente por IA en un 90% automatizados.", leader: "Poder Judicial Dominicano", model: "Algoritmo Preventivo de Conciliación Rápida", sDesc: "Cargar contratos Pymes directamente en el portal oficial que los interpreta frente a impagos de inmediato sentenciando retenciones de cuentas sin durar tres o cuatro años trabados agilizando drásticamente la inversión empresarial por seguridad de cobro digitalizada estricta." },
    { title: "Rastreo Constante del Enriquecimiento Público", desc: "API en la declaración patrimonial frente al Banco Central vivo transparente.", leader: "Cámara de Cuentas Nacional", model: "Vitrina de Declaraciones Abiertas Indexadas", sDesc: "Conexión directa: cuando el sistema halle transferencias sin coincidir o la compra inexplicable de casas bajo parentela cercana lo congela automáticamente al mes en la base de datos nacional alertando sin piedad ni amparo penal exhumable a la fiscalía de perjurio oficial en gestión presencial detectada por inteligencia artificial fiscalizada internacionalmente." },
    { title: "Identidad Digital Estructurada Unívoca", desc: "Cédula absorbe todas licencias e historiales de golpe en billeteras celular digitales.", leader: "Junta Central Electoral", model: "Ecosystem Nacional ID Biométrico Avanzado", sDesc: "Unificación biométrica donde un código QR infalsificable vivo posee tu vacuna, tipo de sangre, multas pagadas y derecho al crédito evitando estafas mercantiles o de suplantación médica e indemnizatoria nacional." },
    { title: "Presupuesto Naciona Cero - Basado Analíticamente (ZBB)", desc: "El presupuesto se reinicia desde cero al anuario sin indexaciones pasadas heredadas de paracaídas inoperantes.", leader: "Presupuesto y Hacienda", model: "Zero-Based Budget Macro-Framework", sDesc: "Eliminación de aumentos graduales: cada ministerio debe sustentar de nuevo cada lápiz que pide al legislativo probando retorno y uso justificado de los pasados 365 días extirpando despilfarros presupuestos arrinconados oscuros y sub-ejecuciones sospechosas sistemáticas." },
    { title: "Auditoría Sistémica de Nóminas IA ('Caza-Botellas')", desc: "Poda del clientelismo estatal masivo inerte.", leader: "Contraloría de la República", model: "Meta Análisis Centralizado Cívico-Laboral", sDesc: "Redes que verifican IP's, ingresos del transporte, marcaje y declaraciones solapadas descubriendo miles de cobrar sin trabajar erradicándo su nombre e inhabilitando cuenta a la brevedad devolviendo en el acto los millones recuperados para redirigirlos a fondos verdaderos y tangibles educativos y estructurales de asfalto y energía urgente prioritaria al amparo y vista de la ciudadanía en portal nacional." },
    { title: "Voto Digital Encriptado Open Source", desc: "Padrón inalterable desde la billetera electrónica personal evitando el clientelismo electoral callejero manipulado y atraso estructural logístico enorme de colegios y acarreadores territoriales inmorales y anticuados a nivel caribeño e internacional general y abarcativo incluyente sin fraudes.", leader: "JCE - Innovación Cívica y Académica", model: "Terminal de Sufragio Celular Estricto Unificado", sDesc: "Adopción de infraestructura Estonia adaptando seguridad local para emitir sufragio vía validación dactilar facial evitando mesas clientelares barriales suprimiendo por fin robos y retrasos en conteos inaceptables para este siglo insertando democracia cristalina madura indudable inmediata imbatible barata digital y transparente." }
  ]
};

const file = require('fs').readFileSync('src/data.ts', 'utf8');
let finalCode = file;

Object.keys(contentPlan).forEach(sector => {
   let sectorSearch = "id: '" + sector + "'";
   if (sector === 'finance') {
     sectorSearch = "id: 'justice'"; 
   }
   
   let idx = finalCode.indexOf(sectorSearch);
   if(idx !== -1) {
      let probsBlock = finalCode.indexOf('problems: [', idx);
      if (probsBlock === -1) return;
      
      let curr = probsBlock;
      for (let i = 0; i < contentPlan[sector].length; i++) {
         let item = contentPlan[sector][i];
         let nextId = finalCode.indexOf("id: '", curr);
         if (nextId === -1) break;
         
         let tIdx = finalCode.indexOf("title: ", nextId);
         let titleQuote = finalCode.charAt(tIdx + 7);
         let tEnd = finalCode.indexOf(titleQuote + ",", tIdx + 8);
         if (tIdx !== -1 && tEnd !== -1) {
            finalCode = finalCode.substring(0, tIdx) + "title: '" + item.title + "'," + finalCode.substring(tEnd + 2);
         }
         
         let dIdx = finalCode.indexOf("description: ", nextId);
         let dQuote = finalCode.charAt(dIdx + 13);
         let dEnd = finalCode.indexOf(dQuote + ",", dIdx + 14);
         if (dIdx !== -1 && dEnd !== -1 && dIdx < finalCode.indexOf("solution: ", nextId)) {
            finalCode = finalCode.substring(0, dIdx) + "description: '" + item.desc + "'," + finalCode.substring(dEnd + 2);
         }

         let solIdx = finalCode.indexOf("solution: {", nextId);
         if (solIdx === -1) break;

         let mIdx = finalCode.indexOf("model: ", solIdx);
         let mQuote = finalCode.charAt(mIdx + 7);
         let mEnd = finalCode.indexOf(mQuote + ",", mIdx + 8);
         if (mIdx !== -1 && mEnd !== -1) {
            finalCode = finalCode.substring(0, mIdx) + "model: '" + item.model + "'," + finalCode.substring(mEnd + 2);
         }

         let lIdx = finalCode.indexOf("leader: ", solIdx);
         let lQuote = finalCode.charAt(lIdx + 8);
         let lEnd = finalCode.indexOf(lQuote + ",", lIdx + 9);
         if (lIdx !== -1 && lEnd !== -1) {
            finalCode = finalCode.substring(0, lIdx) + "leader: '" + item.leader + "'," + finalCode.substring(lEnd + 2);
         }

         let sdIdx = finalCode.indexOf("description: ", solIdx);
         let sdQuote = finalCode.charAt(sdIdx + 13);
         let sdEnd = finalCode.indexOf(sdQuote + ",", sdIdx + 14);
         if (sdIdx !== -1 && sdEnd !== -1) {
            finalCode = finalCode.substring(0, sdIdx) + "description: '" + item.sDesc.replace(/'/g, "\\'") + "'," + finalCode.substring(sdEnd + 2);
         }
         
         curr = finalCode.indexOf("},", solIdx) + 2;
      }
   }
});

require('fs').writeFileSync('src/data.ts', finalCode);
